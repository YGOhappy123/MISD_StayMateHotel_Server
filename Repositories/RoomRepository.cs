...
namespace server.Repositories
{
    public class RoomRepository : IRoomRepository
    {
                ...
                if (!string.IsNullOrWhiteSpace(value))
                {
                    switch (filter.Key)
                    {
                        ...
                        case "maxPrice":
                            ...
                            break;
                        case "isAvailable":
                            query = query.Where(rm => value == "1" ? rm.IsUnderMaintenance == false : rm.IsUnderMaintenance == true);
                            break;
                        case "features":
        ...
        public async Task<bool> CheckIfRoomIsOccupied(int roomId)
        {
            var currentlyOccupiedRoomIds = await _dbContext
                .Bookings.Where(bk => bk.Status == BookingStatus.CheckedIn || bk.Status == BookingStatus.CheckedOut)
                .SelectMany(bk => bk.BookingRooms.Select(br => br.RoomId))
                .Distinct()
                .ToListAsync();

            return currentlyOccupiedRoomIds.Any(id => id == roomId);
        }
